function [vr] = initializeFreeMaze(vr)
...
% Initial values for subfields in the 'vr' structure array
    ...
    vr.rotaryJoystick = rotaryEncoder(vr.arduinoBoard,'D2','D3',2000);                                  % rotary encoder                                                                     % threshold for decision
        ...
        elseif vr.propertyValues{2} == 4
            vr.countsThreshold = 40;
        elseif vr.propertyValues{2} == 5
            vr.countsThreshold = 40;
        elseif vr.propertyValues{2} == 6
            vr.countsThreshold = 40;
        elseif vr.propertyValues{2} == 10
            vr.countsThreshold = 40;
        elseif vr.propertyValues{2} == 20
            vr.countsThreshold = 40; %50; 
        end
    ...
    if strcmpi(vr.propertyValues{8},'on')
        writeDigitalPin(vr.arduinoBoard,'D6',0)                                                         % LED off
        writePWMVoltage(vr.arduinoBoard,'D5',4.5660)                                                    % LED power
        %  0.5 mW --- 0.0200
        % 10.0 mW --- 0.9009
        % 20.0 mW --- 1.8091
        % 30.0 mW --- 2.7248
        ...
        vr.optoStimThreshold = 0.50;
        vr.optogeneticsTrials = [];
    end
    ...
    if strcmpi(vr.mazeType,'LandR') || strcmpi(vr.mazeType,'FreeLandR')
