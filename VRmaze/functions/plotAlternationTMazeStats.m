function plotAlternationTMazeStats(behaviorData)
%% Settings
...
%% Plot
        ...
        s = behaviorData.sisterMaze(window);
        x1 = find(f==1);        % free
        ...
        x8 = find(f==0);        % blocked
        x9 = find(s==1);        % maze with two arms
        ...
        % Lickometer
        resampledSpoutTouch = [];
        for i=1:size(behaviorData.spoutTouch(:,window),2)
        ...
        % Performance
            ...
            % Free decisions in sister maze with two arms only
            y1 = intersect(x1,x2); y1 = intersect(y1,x3); y1 = intersect(y1,x9); y1 = length(y1);     % free correct decision
            y2 = intersect(x1,x2); y2 = intersect(y2,x4); y2 = intersect(y2,x9); y2 = length(y2);     % free wrong decision
            y3 = intersect(x1,x3); y3 = intersect(y3,x5); y3 = intersect(y3,x9); y3 = length(y3);     % free correct left
            y4 = intersect(x1,x3); y4 = intersect(y4,x6); y4 = intersect(y4,x9); y4 = length(y4);     % free correct right
            y5 = intersect(x1,x4); y5 = intersect(y5,x5); y5 = intersect(y5,x9); y5 = length(y5);     % free wrong left
            y6 = intersect(x1,x4); y6 = intersect(y6,x6); y6 = intersect(y6,x9); y6 = length(y6);     % free wrong right
            ...
            clear('y1','y2','y3','y4','y5','y6')
                ...
                if isempty(x1) || isempty(x9)
                    freeDecisionCorrectFraction = allTrialsCorrectFraction;
                    freeDecisionWrongFraction = allTrialsWrongFraction;
                    freeDecisionCorrectLeft = allTrialsCorrectLeft;
                    freeDecisionCorrectRight = allTrialsCorrectRight;
                    freeDecisionWrongLeft = allTrialsWrongLeft;
                    freeDecisionWrongRight = allTrialsWrongRight;
                end
    ...
    figure, set(gcf,'Position',[160 120 1600 800])
        ...
        lickometer = subplot(4,4,[1 5 9 13]);
            colors = [1 1 1;0.6 0.6 0.6;1 0 0; 0 0 1;1 0.5 0.5;0.5 0.5 1];
            ...
            hold on
                ...
                plot((numberOfBins/trialDuration)*[3 3],[0 behaviorData.totalTrials+0.5],'Color',[0.6 0.6 0.6])
                plot((numberOfBins/trialDuration)*[5 5],[0 behaviorData.totalTrials+0.5],'Color',[0.6 0.6 0.6])
                scatter((numberOfBins/trialDuration)*(behaviorData.decisionDelay(intersect(x2,x3),1)+3),intersect(x2,x3),10,[0 0.9 0],'filled')     % wrong right
                scatter((numberOfBins/trialDuration)*(behaviorData.decisionDelay(intersect(x2,x4),1)+3),intersect(x2,x4),10,[0.9 0 0.9],'filled')   % wrong left
            hold off
        ...
        pieChart = subplot(4,4,[3 7]);
            colors = [1 0 0; 0 0 1;1 0.5 0.5;0.5 0.5 1;1 1 1];
        ...
        subplot(4,4,[10 14]);
            % Data
            reward = behaviorData.reward*0.001*cumsum(c);   % in mL
        ...
        subplot(4,4,[2 6])
            ...
            hold on
                plot(a1,'Color',[1 0 0])
                plot(a2,'Color',[0 0 1])
                plot(a3,'Color',[1 0.5 0.5])
                plot(a4,'Color',[0.5 0.5 1])
                plot(a5,'Color',[0.6 0.6 0.6])
                y = ylim;
                if ~isempty(idx)
                    plot([idx idx],[0 y(2)],'--','Color',[1 0.5 0])
                end
            hold off
            ...
            text(0.1*length(a1),0.85*y(2),'wrongLeft','FontSize',10,'Color',[1 0.5 0.5],'HorizontalAlignment','left')
            text(0.1*length(a1),0.80*y(2),'wrongRight','FontSize',10,'Color',[0.5 0.5 1],'HorizontalAlignment','left')
            text(0.1*length(a1),0.75*y(2),'noResponse','FontSize',10,'Color',[0.6 0.6 0.6],'HorizontalAlignment','left')
            clear('a1','a2','a3','a4','a5','y','idx')
        
        % Trials with responses and free decision in maze with two arms
            % Response delay (both mazes)
            subplot(4,4,[4 8])
                hold on
                ...
                hold off
                set(gca,'Box','off','TickDir','out','XTick',[],'XLim',[0 2],'YLim',[0 2])      
                ylabel('response delay, time (s)','FontSize',12,'FontWeight','bold')
            
        % Decision ratio (maze with two arms only)
        pieChart = subplot(4,4,[11 15]);
            colors = [1 0 0; 0 0 1;1 0.5 0.5;0.5 0.5 1];
        ...
        % Correct/wrong ratio (maze with two arms only)
