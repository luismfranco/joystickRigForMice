function [vr] = runtimeFreeMaze(vr)
...
% Evaluates whether to move forward, to wait for a decision, to turn after a decision is made or to finalize and restart trial
    ...
    % Position
    if strcmpi(vr.mazeType,'FreeLandR')
        vr.trialLocation = cat(1,vr.trialLocation,[vr.position(1) vr.position(2) vr.position(4)]);
    end
        ...
        if vr.stopConditional*vr.position(vr.axisDisplacement(vr.trialMazeContext+1)) > abs(vr.decisionPointCoordinates(vr.trialMazeContext+1,vr.axisDisplacement(vr.trialMazeContext+1)))-vr.decisionPointCornerDistance
            ...
            elseif vr.decisionPointArrival == 1
                ...
                if isempty(vr.lastDecision) && vr.timeElapsed - vr.decisionTimeStart > vr.timeThresholdForDecision
                    ...
                    vr.restartTrial = 1;
                    % Turn optogenetics stimulation off
                ...
                if vr.position(4) >= vr.viewAngle+(1/4)*pi && vr.position(4)< vr.viewAngle+(3/4)*pi         % between  45° and 135°
                    vr.lastDecision = 0;    % left
                elseif vr.position(4) <= vr.viewAngle+(7/4)*pi && vr.position(4) > vr.viewAngle+(5/4)*pi    % between 225° and 315°
    ...
    if vr.turnSpinMovement==1
        if vr.timeElapsed-vr.startTurnTime <= vr.turnTime
            ...
            elseif vr.lastDecision==1
                vr.position(4) = vr.viewAngleAtDecision + ((3/2*pi-vr.viewAngleAtDecision)*((vr.timeElapsed-vr.startTurnTime)/vr.turnTime));
            end
    ...
    if vr.restartTrial == 1
        ...
        if vr.timeElapsed-vr.timeOutStart > vr.trialTimeOut
            ...
            % Free navigation maze (open both servos)
            if strcmpi(vr.mazeType,'FreeLandR')
                writePosition(vr.servoLock1,vr.servoLimits(1,2));       % open servo lock 1
                writePosition(vr.servoLock2,vr.servoLimits(2,2));       % open servo lock 2
            end
            % Start optogenetics stimulation
            if strcmpi(vr.propertyValues{8},'on')
                vr.optoStimProbability = rand(1);
                if vr.freeDecisionProbability <= vr.freeDecisions && vr.optoStimProbability <= vr.optoStimThreshold
                    writeDigitalPin(vr.arduinoBoard,'D6',1)
                    vr.optogeneticsTrials = cat(1,vr.optogeneticsTrials,1);
                else
                    vr.optogeneticsTrials = cat(1,vr.optogeneticsTrials,0);
                end
            end
        end
    end
