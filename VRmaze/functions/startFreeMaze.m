function startFreeMaze(mazeType,mouseName,varargin)
...
% Properties
propertyNames = {'Duration','Rig','Reward','FreeDecisions','Servo1Lim','Servo2Lim','Imaging','Optogenetics','FreeRoute'};
propertyValues = {600,0,10,1,[1 0.6],[0 0.4],'off','off','off'};
...
elseif propertyValues{2} > 0 && propertyValues{2} <= 6
...
if propertyValues{2} == 0
    propertyValues{5} = [1 0.50];
    propertyValues{6} = [0 0.40];
elseif propertyValues{2} == 1
    propertyValues{5} = [1 0.70];
    propertyValues{6} = [0 0.30];
elseif propertyValues{2} == 2
    propertyValues{5} = [1 0.65];
    propertyValues{6} = [0 0.30];
elseif propertyValues{2} == 3
    propertyValues{5} = [1 0.65];
    propertyValues{6} = [0 0.50];
elseif propertyValues{2} == 4
    propertyValues{5} = [0.98 0.70];
    propertyValues{6} = [0.02 0.30];
elseif propertyValues{2} == 5
    propertyValues{5} = [1 0.65];
    propertyValues{6} = [0 0.40];
elseif propertyValues{2} == 6
    propertyValues{5} = [1 0.65];
    propertyValues{6} = [0 0.20];
elseif propertyValues{2} == 10
    propertyValues{5} = [1 0.70];
    propertyValues{6} = [0 0.40];
elseif propertyValues{2} == 20
...
if exist([experimentRootDir,datestr(date,'yymmdd'),'\',mouseName,'.mat'],'file') > 0
    ...
    if strcmpi(propertyValues{8},'on')
        if propertyValues{2} == 10
            behaviorData.optogeneticsTrials = [];
        end
    end
end
...
elseif propertyValues{2}==10
    load(['D:\Dropbox\BehaviorRig\Luis\behaviorRigCodes\experiments\',mazeType,'.mat'])
else
